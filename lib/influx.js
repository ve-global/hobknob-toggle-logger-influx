...
var formatForInflux = (payload) => {
  var metricName = `${prefix}hobknob.${payload.applicationName}`;
  var toggle = !!payload.toggleName ? `${payload.featureName}/${payload.toggleName}` : payload.featureName;
  var user = getUsername(payload.user);
  return `${metricName},toggle=${toggle},user=${user}${tags} value="${payload.value}" ${(new Date()).getTime()}`;
};
